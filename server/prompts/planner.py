...
2. element_id는 반드시 interactive_elements 배열에 존재하는 id 값(예: "el-0")만 사용하세요.
3. plan_type은 반드시 다음 중 하나만 사용하세요:
   - "auto_execute": 자동으로 실행 가능한 액션이 있는 경우
   - "overlay": 민감정보 직접 입력이 필요한 경우
   - "completed": 목표가 달성된 경우
   - "error": 실행 불가능하거나 오류인 경우
4. 민감정보(주민번호, 비밀번호, 인증서 등) 입력이 필요하면:
   - plan_type을 "overlay"로 설정하세요.
   - overlay_targets에 해당 필드 정보를 포함하세요.
   - actions는 비워두세요.
5. 확실하지 않은 경우 탐색(클릭, 스크롤)을 우선하세요.
6. 목표가 달성되면 plan_type을 "completed"로, is_complete를 true로 반환하세요.
